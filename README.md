...
`mermaid-zoom` 会在 Typora 渲染后的 Mermaid 图表上添加一个放大按钮。点击按钮后会打开图表预览弹层,适合查看较大的流程图、时序图或结构图。当前版本为 `0.2.0`。

主要特性:

- 自动识别 Typora 中渲染后的 Mermaid SVG,并在图表右上角添加悬浮放大按钮。
- 鼠标悬停在 Mermaid 图上时按 `F` 键可直接打开预览弹层(自 `0.2.0` 起支持)。
...
- 主色取自 Claude 标志色 `#D97757`,配合低饱和的米白/暖灰底色(`#faf9f5`)与深色背景(`#262624`),整体观感克制、偏暖。
- 字体策略:正文优先使用「霞鹜文楷」,代码块优先 `Fira Code` / `Source Code Pro`,中文回退 `PingFang SC` / `Microsoft YaHei`;macOS 之外走 `Consolas` 兜底。
- 圆角、阴影、按钮、输入框等控件统一通过 `--box-shadow-userinput` 系列阴影 + 圆角变量打造,带轻量悬浮反馈。
- 主题对插件生态做了适配:为 `#plugin-action-buttons` 提供与侧边栏统一的字体色与边框,源视图切换按钮背景透明化,导出面板选中态与侧边栏选中态保持一致。
- 焦点模式(`F8`)下,非聚焦块统一降至 28% 透明度,聚焦图片/SVG 保持清晰,有助于在长文里突出当前段落。
- 内置响应式布局:在 `unibody-window` 主题预览网格下,按 `1440 / 1200 / 992 / 768` 断点切换 4/3/2/1 列,小窗口下不至于挤压。
- 提供 `print` 媒体查询样式,导出 PDF 时自动调整列表/引用/代码块等元素间距,避免被打印样式覆盖。
